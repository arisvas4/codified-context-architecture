...
# From the case study project case study (real production script, 250 lines)
#
# WHAT IT DOES: 9 cross-reference checks ensuring CLAUDE.md, MCP server.py,
# context docs, and agent specs all reference each other correctly.
#
# WHY IT MATTERS: Without validation, references go stale silently. A context
# doc that references a deleted source file gives agents wrong information.
# An agent listed in CLAUDE.md but missing its AGENT.md file fails silently.
#
# HOW TO ADAPT: Modify the CONFIGURATION section (lines 10-15) with your
# project's paths. The 9 checks are generic and work with any project that
# follows the .claude/context/ + .claude/agents/ + MCP server.py structure.
#
# The checks:
#   1. CLAUDE.md -> context doc references (broken links)
#   2. CLAUDE.md -> agent references (missing agents)
#   3. MCP server.py -> context doc references (stale entries)
#   4. MCP server.py -> agent references (stale entries)
#   5. Context doc -> context doc cross-references (broken links)
#   6. Orphaned context docs (unreferenced by CLAUDE.md or MCP)
#   7. Version headers on context docs (for drift tracking)
#   8. References footer validation (source file existence)
#   9. CLAUDE.md Key Files Reference (source file existence)
# ============================================================================
#
