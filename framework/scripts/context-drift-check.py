...
From the case study project case study (real production script, 498 lines).

WHAT IT DOES: Runs automatically at session start (via Claude Code hooks).
Checks recent git commits to detect when code changes happen without
corresponding context doc updates, and when debugging-heavy sessions
might warrant doc/agent updates.

WHY IT MATTERS: Context docs go stale as code evolves. Without automated
detection, agents work from outdated information. This script bridges the
gap by flagging drift at the moment the developer starts a new session.

HOW TO ADAPT:
1. Update SUBSYSTEM_PRIORITY dict with your subsystems
2. Update DEBUG_KEYWORDS for your domain
3. Wire into Claude Code hooks (see case-study/hooks-config.json)

PRIORITY TIERS (determines agent behavior when drift is detected):
- HIGH: Agent auto-updates the flagged context doc before starting work
- MEDIUM: Agent mentions the drift and asks the user
- LOW: Suppressed (not shown)

AUTO-DISMISS: Warnings auto-dismiss after being shown twice without new
commits, preventing noise in long-running projects.
